...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (err?.code === "P2002") {
      return NextResponse.json({ error: "A record with this name already exists at this level" }, { status: 409 });
    }
    if (err?.code === "P2003") {
      return NextResponse.json({ error: "Invalid parent reference" }, { status: 400 });
    }
  ...
  try {
    ...
    switch (level) {
      case "category":
        ...
        updated = await prisma.category.update({ where: { id }, data: { name: name.trim() } });
        break;
      case "subject":
...
export async function DELETE(req: NextRequest) {
  ...
  if (force && user.role !== "SUPER_ADMIN") {
    return NextResponse.json({ error: "Only SUPER_ADMIN can force delete" }, { status: 403 });
  }
  ...
  try {
    ...
    switch (level) {
      case "category": {
        ...
        if (force) {
          await prisma.subtopic.deleteMany({
            where: { topic: { subject: { categoryId: id } } },
          });
          await prisma.topic.deleteMany({
            where: { subject: { categoryId: id } },
          });
      ...
      case "topic": {
        ...
        if (!record) return NextResponse.json({ error: "Not found" }, { status: 404 });
        if (force) {
          await prisma.subtopic.deleteMany({ where: { topicId: id } });
        }
    ...
    if (err?.code === "P2003") {
      return NextResponse.json(
        { error: "Cannot delete: taxonomy is in use. Use force=true with SUPER_ADMIN to cascade delete." },
        { status: 409 }
      );
    }
