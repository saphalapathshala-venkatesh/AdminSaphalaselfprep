...
const ACTIVITY_CONFIG: Record<string, { label: string; icon: string; bg: string; color: string }> = {
  ...
  GLOBAL_DEVICE_RESET:       { label: "Global Device Reset", icon: "⚠️", bg: "#fef3c7", color: "#92400e" },
...
function activityBadge(type: string) {
  const cfg = ACTIVITY_CONFIG[type] || { label: type, icon: "•", bg: "#f1f5f9", color: "#475569" };
  return (
    <span style={{ padding: "2px 10px", borderRadius: "10px", fontSize: "0.75rem", fontWeight: 700, background: cfg.bg, color: cfg.color, whiteSpace: "nowrap" }}>
      {cfg.icon} {cfg.label}
    </span>
  );
}
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        {user && (
          <p style={{ margin: "0.25rem 0 0", fontSize: "0.875rem", color: "#64748b" }}>
            {user.name || user.email} · {total} events
          </p>
        )}
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Event", "Device / IP", "Details", "Time"].map(h => (
              ...
              <tbody>
                {activities.map(a => (
                  <tr key={a.id}
                    ...
                    <td style={{ padding: "0.75rem 1rem", borderBottom: "1px solid #f1f5f9" }}>{activityBadge(a.activityType)}</td>
                    <td style={{ padding: "0.75rem 1rem", borderBottom: "1px solid #f1f5f9", fontSize: "0.75rem", color: "#64748b" }}>
                      {a.deviceKey && <div><code style={{ background: "#f1f5f9", padding: "0 4px", borderRadius: 3 }}>{a.deviceKey.slice(0, 10)}…</code></div>}
                      {a.ipAddress && <div style={{ color: "#94a3b8" }}>{a.ipAddress}</div>}
                    </td>
                    <td style={{ padding: "0.75rem 1rem", borderBottom: "1px solid #f1f5f9", fontSize: "0.75rem", color: "#94a3b8", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {parseMeta(a.meta) || "—"}
                    </td>
