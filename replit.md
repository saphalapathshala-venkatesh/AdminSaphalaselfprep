...
- **Core Features**:
    ...
    - **Doubts Module**: New `DoubtStatus` enum (`OPEN | ANSWERED | CLOSED`) and `Doubt` model (`id, tenantId, studentId→User, videoId?→Video, courseId?→Course, question, answer?, status, answeredById?→User, createdAt, updatedAt`) added to schema. API routes: `GET/POST /api/admin/doubts` (list with status/search/video/course filters, paginated) and `GET/PATCH/DELETE /api/admin/doubts/[id]` (reply, close, reopen, delete). Admin page `/admin/doubts`: table with Status, Student, Question preview, Video/Course context, Date, and inline expandable Reply panel with textarea + status selector + Save. Dashboard gains a "Student Doubts" stat card (total/open/answered/closed counts) with a "Manage Doubts →" link. Nav entry "Doubts" added under ENGAGEMENT.
