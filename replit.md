...
- **Core Features**:
    ...
    - **Cashfree Payment Infrastructure**: Full payment integration using Cashfree PG V3 (API version 2023-08-01). `PaymentConfig` model stores merchant credentials (DB-managed, admin-switchable; only one `isActive=true` at a time). `PaymentOrder` model tracks the full payment lifecycle (CREATED → PENDING → PAID/FAILED/CANCELLED). `lib/payment/types.ts` defines the `PaymentProvider` interface. `lib/payment/cashfree.ts` implements Cashfree order creation, verification, and webhook signature validation (HMAC-SHA256). `lib/payment/index.ts` is the abstraction layer that fetches the active config and returns the correct provider. Student API: `POST /api/student/orders` creates an order (backend-calculates trusted amount, applies coupon), `GET /api/student/orders/[id]` polls status and settles PAID orders (creates Purchase + activates UserEntitlements). `POST /api/webhooks/cashfree` is the idempotent webhook endpoint (signature-verified, processes PAYMENT_SUCCESS and PAYMENT_FAILED events). `GET /api/admin/orders` is the admin orders dashboard API. Admin UI: `/admin/payment-config` (SUPER_ADMIN only — add/edit/activate/delete configs, masked secrets), `/admin/orders` (all orders with status badges, search, pagination). Credentials are added from the admin UI — no hardcoded env vars. Webhook URL: `/api/webhooks/cashfree`.
