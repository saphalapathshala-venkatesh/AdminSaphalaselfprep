...
- **Core Features**:
    ...
    - **Video XP Rewards**: `Video` model gains `xpEnabled Boolean @default(false)` and `xpValue Int @default(0)`. Videos list page (`/admin/videos`) now shows inline XP toggle (ON/OFF checkbox) and XP value input per row with a "Save" button that appears only when a change is pending; uses `PUT /api/videos/[id]` with `xpEnabled`/`xpValue`. Videos API `GET /api/videos` now also supports `?provider=` filter. Provider filter dropdown added alongside the existing status filter.
